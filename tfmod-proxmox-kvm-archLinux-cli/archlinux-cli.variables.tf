...
variable "proxmox_vm_template_tags" {
  ...
  default     = ["archlinux", "cli", "minimal", "template"]
}

variable "proxmox_vm_tags" {
  type        = list(string)
  description = "Tags to assign to created Proxmox VMs"
  default     = ["archlinux", "cli", "minimal", "terraform"]
}
...
variable "memory_size" {
  type        = string
  description = "Memory size in GB/MB for the cloned/created VM"
  default     = "512M"
  validation {
...
variable "disk_size_boot" {
  type        = string
  description = "Boot disk size in GB for the created VM. Default is '8G'."
  default     = "8G"
  validation {
...
variable "disk_size_extra" {
  type        = string
  description = "Extra disk size in GB for the created VM. Default is '0M' (Not created)."
  default     = "0M"
  validation {
    condition     = (tonumber(replace(var.disk_size_extra, "G", "")) >= 0 && tonumber(replace(var.disk_size_extra, "G", "")) <= 200 ) || (tonumber(replace(var.disk_size_extra, "M", "")) >= 0 && tonumber(replace(var.disk_size_extra, "M", "")) <= 204800)
    error_message = "Extra disk size must be at least '>0G'/ '>0M' , and at most '200G' / '204800M'. Also note that is a String value inside Double-Quotes with 'G' or 'M' at its end."
  }
}

variable "disk_extra_ssd_enabled" {
  type        = bool
  description = "Enable SSD flag for the extra disk of the created VM"
  default     = false
  validation {
    condition     = var.disk_extra_ssd_enabled == true || var.disk_extra_ssd_enabled == false
    error_message = "Disk_extra_ssd_enabled must be a boolean value (true or false)."
  }
} 
...
variable "rsyslog_yay_aur_installed" {
  type    = bool
  description = "Whether to install 'yay' AUR helper and 'rsyslog' during the build process"
  default = false
  validation {
    condition     = var.rsyslog_yay_aur_installed == true || var.rsyslog_yay_aur_installed == false
    error_message = "rsyslog_yay_aur_installed must be a boolean value (true or false)."
  }
